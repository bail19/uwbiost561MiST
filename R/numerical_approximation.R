#' Approximate Noncentral chisq distribution for the weighted sum of independent chisq distributions
#'
#'This function calculates the df and the noncentral parameter of an approximated noncentral chisq distribution for the weighted sum of independent chisq distributions.
#'
#' @param weight A vector of weights for each chisq distribution.
#' @param df A vector of df for each chisq distribution, default: 1 for each weight
#'
#' @return A vector of mean, sd of the weighted sum, the df(\code{l}) and
#' the noncentrality parameter of the approximation noncentral chisq
#' @export
#'
#' @examples
#' # Define weights for the chi-squared distributions
#' weights <- c(0.5, 1.5, 2.0)
#'
#' result <- WeightSumChisq(weight = weights)
#' result
...
#' Calculate p-Value for weighted sum of chisq Distributions
#'
#' This function calculates the p-value for a given test statistic \code{Q} that is a weighted sum of chisq distributions.
#' It uses Liu's(2009) approximation method to adjust \code{Q} and calculates the p-value based on the noncentral chisq.
#'
#' @param Q A test statistic for which the p-value is to be calculated.
#' @param weight A vector of weights for each chisq
#' @param method The method used for approximation,
#' default: "3M". Other options include: "4M" which is a modified version of the \code{WeightSumChisq} function.
#' @param df A vector of df for each chisq, default: 1 for each weight
#'
#' @return A list containing the \code{pvalue}, the approximated \code{df}, and the noncentrality parameter \code{ncp}
#' @export
#'
#' @examples
#' # Define the statistic Q, weights, and degrees of freedom for the chi-squared distributions
#' Q = 10.5
#' weights = c(0.5, 1.5, 2.0)
#' dfs = c(1, 1, 1) # Assuming all chi-squared distributions have 1 degree of freedom
#'
#' result = pvalue.liu(Q, weights, df = dfs)
#' result
...
quan_rho_calculator = function(grid=seq(0,1,by=0.05),df_f,lambda_r,min_pvalue,acc=5e-10,lim=1e4,max_core=4){
  ...
  quan_rho_grid_inner = parallel::mclapply(grid_inner,FUN=function(grid_point){
...
pvalue_minimizePValue_davies = function(u_f,u_r,df_f,lambda_r,min_pvalue=NULL,quan_grid=NULL,acc=5e-10,lim=1e4,ChisqApp="3M",max_core=4){
  ...
  integrand = function(a,min_pvalue,df_f,lambda_r,acc=5e-10,grid=NULL,quan_rho_grid_inner=NULL,max_core=4){
    ...
    numCores = parallel::detectCores()
    res = parallel::mclapply(a,FUN=function(a.tmp){
      ...
      min_quantile_tmp = optimize(quan_rho,interval=optimize_interval,df_f=df_f,lambda_r=lambda_r,p_rho_min=min_pvalue,u_f=a.tmp)$objective
...
pvalue_minimizePValue_liu = function(u_f,u_r,df_f,lambda_r,min_pvalue=NULL,acc=5e-10,lim=1e4,ChisqApp="4M", pvalue.r=pvalue.r,pvalue.f=pvalue.f, weight=weight){
  ...
  integrand = function(a,min_pvalue,df_f,lambda_r,acc=5e-10){
    ...
    res = sapply(a,function(a.tmp){
      ...
      min_quantile_tmp = optimize(quan_rho,interval=c(0,1),df_f=df_f,lambda_r=lambda_r,p_rho_min=min_pvalue,u_f=a.tmp)$objective
      min_quantile = min(quan_rho(rho=0,df_f=df_f,lambda_r=lambda_r,p_rho_min=min_pvalue,u_f=a.tmp),
                         min_quantile_tmp)
      tail.prob = 0
      if(class(try(CompQuadForm::davies(q=min_quantile,lambda=lambda_r,acc=acc,lim=1/acc)))!="try-error") tail.prob = CompQuadForm::davies(q=min_quantile,lambda=lambda_r,acc=acc,lim=1/acc)$Qq
